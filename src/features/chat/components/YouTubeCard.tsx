...
export function YouTubeCard({ reference, className }: YouTubeCardProps) {
  ...
  return (
    <motion.button
      ...
      className={cn(
        "group mt-3 block w-full overflow-hidden rounded-2xl border border-border bg-surface text-left",
        "shadow-sm transition-shadow duration-300 hover:shadow-md active:scale-[0.99]",
        className,
      )}
    >
      ...
      <div className="p-3.5">
        ...
        <p className="mt-1 line-clamp-1 text-xs text-mint-dark">
          {reference.label}
        </p>
