...
import {
  SimulationProgress,
} from "@/features/simulate/components/SimulationProgress";
import type {
  ProcedureType,
  SimulateResponse,
...
export function VirtualSimulator() {
  ...
  return (
    <div className="mx-auto flex w-full max-w-lg flex-col gap-6 px-4 py-6 pb-[calc(5rem+env(safe-area-inset-bottom))]">
      ...
      {isLoading && (
        <SimulationProgress messageIndex={loadingIndex} />
      )}
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="rounded-xl bg-mint/5 px-4 py-3 text-xs text-muted">
            <p>
              ...
              {Math.round(result.strength * 100)}% · {result.source === "demo" ? "데모" : "Replicate SD"}
            </p>
          </div>
          <Button
            variant="secondary"
